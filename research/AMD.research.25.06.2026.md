# AMD Research Report
**Advanced Micro Devices, Inc.** — Technical + Fundamental Analysis, Data Snapshot, Institutional Investors
Signed file: `AMD.research.25.06.2026`

| Field | Value |
|---|---|
| Current price | $519.74 (2026-06-25, ~15-min delayed) |
| Market cap | $847.49B |
| Sector / Industry | Technology / Semiconductors |
| Main theme | Semiconductors — see catalysts below |
| Current stance | High technical momentum, mixed fundamentals, value: low (expensive), risk: high. |

> Research journal, not financial advice. For education and decision journaling only — not a recommendation to buy, hold, or sell. Public Yahoo Finance data retrieved 2026-06-25.

## 1. Executive Summary
| Area | Score / Read | Reason |
|---|---|---|
| Technical momentum | High | 1Y +301.10%; price +94.86% vs SMA200. |
| Fresh setup quality | Moderate / wait | -5.78% from 52w high; no clean fresh pivot by default. |
| Fundamental quality | Mixed | Profit margin 13.37%, revenue growth 37.80%. |
| Value attractiveness | Low (expensive) | Forward P/E 39.47, EV/Sales 22.40. |
| Risk level | High | Beta 2.49, ATR 6.3% of price, short float 2.71%. |

**Bottom line:** High technical momentum, mixed fundamentals, value: low (expensive), risk: high.

**Key questions before any trade (standard):**
- Is the stock forming a new tight base, or is it simply extended after news-driven gaps?
- Where is the actual invalidation level? If that level is too far away, the setup is not clean.
- Is the move supported by new institutional demand or just retail/short-covering momentum?
- Can earnings convert fast enough to justify the valuation?

## 2. Company Overview and Recent Catalysts
Advanced Micro Devices, Inc. operates as a semiconductor company internationally. It operates in three segments: Data Center, Client and Gaming, and Embedded.

**Recent headlines (potential catalysts):**

| Headline | Source | Date |
|---|---|---|
| Qualcomm jumps as it looks beyond smartphones to cash in on the AI boom | Yahoo Finance | 2026-06-24 |
| Advanced Micro Devices (AMD) and Rackspace Technology Sign a Definitive Agreement | Insider Monkey | 2026-06-25 |
| Will Nvidia Continue to Dominate in AI? This One Number Offers a Strikingly Clear Answer. | Motley Fool | 2026-06-25 |
| Gartner Names Advanced Micro Devices, Inc. (AMD) “The Company to Beat for Enterprise AI Server CPUs | Insider Monkey | 2026-06-25 |

## 3. Technical Analysis
| Technical item | Read |
|---|---|
| Price vs 52-week range | Close $519.74; 52w high $551.63 (-5.78%); 52w low $134.80 (+285.56%) |
| Trend | +94.86% vs SMA200, +21.35% vs SMA50, +1.84% vs SMA20 |
| Momentum | RSI(14) 56.5 (neutral) |
| Volatility | ATR(14) 32.56 (~6.3% of price); beta 2.49 |
| Setup perspective | -5.78% from 52w high; no clean fresh pivot by default. |

**Performance snapshot:**

| Window | Return |
|---|---|
| Week | +2.5% |
| Month | +11.2% |
| Quarter | +153.1% |
| Half Y | +143.5% |
| 1Y | +301.1% |
| YTD | +132.6% |

## 4. Fundamental Analysis
| Factor | Observation | Implication / extra |
|---|---|---|
| Revenue (ttm) | $37.45B | Revenue growth 37.80% y/y |
| Profitability | Gross 53.06%, operating 14.40%, net 13.37% | ROA 3.65%, ROE 8.06% |
| Balance sheet | Cash $12.35B, debt $3.87B | Current ratio 2.73, debt/equity 6.00 |
| Valuation | P/E 172.67, forward P/E 39.47, P/S 22.63, P/B 13.14 | EV/Sales 22.40, EV/EBITDA 112.92 |
| Growth expectations | Earnings growth 91.20%, EPS q/q 95.10% | Analyst mean target $487.90 (48 analysts) |

## 5. Institutional Investors and Ownership
| Metric | Value |
|---|---|
| Institutional ownership | 72.16% |
| Insider ownership | 0.40% |
| Short float | 2.71% |
| Short ratio (days to cover) | 1.1 |

**Top institutional holders:**

| Holder | Shares | % Out | Reported |
|---|---|---|---|
| Blackrock Inc. | 145,574,284 | 8.93% | 2026-03-31 |
| Vanguard Capital Management LLC | 105,940,266 | 6.50% | 2026-03-31 |
| State Street Corporation | 74,771,220 | 4.59% | 2026-03-31 |
| Geode Capital Management, LLC | 39,791,086 | 2.44% | 2026-03-31 |
| Vanguard Portfolio Management LLC | 37,603,675 | 2.31% | 2026-03-31 |
| Price (T.Rowe) Associates Inc | 28,846,470 | 1.77% | 2026-03-31 |
| JPMORGAN CHASE & CO | 24,248,547 | 1.49% | 2026-03-31 |
| Morgan Stanley | 23,781,608 | 1.46% | 2026-03-31 |
| UBS AM, a distinct business unit of UBS ASSET MANAGEMENT AMERICAS LLC | 19,192,764 | 1.18% | 2026-03-31 |
| Northern Trust Corporation | 16,427,217 | 1.01% | 2026-03-31 |

## 6. Risk Review and Setup Plan
- **Valuation risk:** Forward P/E 39.47, EV/Sales 22.40. Multiple compression is the main downside if growth disappoints.
- **Volatility risk:** Beta 2.49, ATR 6.3% of price, short float 2.71%. Size positions accordingly.
- **Technical trap:** near 52-week highs — do not confuse an old breakout that already worked with a fresh entry.
- **Macro/sector risk:** semis are high-beta to AI capex sentiment, rates, and export-control headlines.

### Setup checklist for your journal
| Question | Pass condition |
|---|---|
| Is there a new base? | Tight sideways action near highs, declining volume, clear resistance. |
| Is the pivot obvious? | A clean level that multiple traders can see; not a random candle high. |
| Is risk defined? | Stop based on structure and less than the expected first upside target. |
| Is volume confirming? | Breakout volume expands; pullback volume dries up. |
| Is position size small enough? | A normal shakeout should not cause emotional decision-making. |

## 7. Full Data Snapshot
| Metric | Value | Metric | Value |
|---|---|---|---|
| Price | $519.74 | EV/Sales | 22.40 |
| Market cap | $847.49B | EV/EBITDA | 112.92 |
| Beta | 2.49 | Gross margin | 53.06% |
| RSI(14) | 56.5 | Operating margin | 14.40% |
| ATR(14) | 32.56 | Profit margin | 13.37% |
| SMA20 dist | +1.84% | ROA | 3.65% |
| SMA50 dist | +21.35% | ROE | 8.06% |
| SMA200 dist | +94.86% | Revenue (ttm) | $37.45B |
| 52W high | $551.63 | Revenue growth y/y | 37.80% |
| 52W low | $134.80 | Inst. ownership | 72.16% |
| P/E (ttm) | 172.67 | Insider ownership | 0.40% |
| Forward P/E | 39.47 | Short float | 2.71% |
| PEG (trailing) | 1.25 | Avg volume | 37,556,846 |
| P/S | 22.63 | Employees | 31,000 |
| P/B | 13.14 | Analyst rec (1=buy..5=sell) | 1.5 |

## 8. Analyst Actions
| Date | Action | Firm | Rating change |
|---|---|---|---|
| 2026-06-24 | main | UBS | Buy → Buy |
| 2026-06-12 | up | Citigroup | Neutral → Buy |
| 2026-06-11 | main | B of A Securities | Buy → Buy |
| 2026-06-01 | main | Barclays | Overweight → Overweight |
| 2026-06-01 | main | TD Cowen | Buy → Buy |
| 2026-06-01 | main | Mizuho | Outperform → Outperform |
| 2026-05-19 | main | Evercore ISI Group | Outperform → Outperform |
| 2026-05-18 | main | Citigroup | Neutral → Neutral |

## 9. Conclusion
AMD: High momentum / mixed fundamentals / low (expensive) value / high risk. Per the journal method: act only on a defined setup (new tight base, controlled pullback, or confirmed reclaim of a key level) with risk sized in advance — never chase an extended move.

## Sources
| Source | URL | Used for |
|---|---|---|
| Yahoo Finance quote/profile/statistics for AMD | https://finance.yahoo.com/quote/AMD | Snapshot metrics, ownership, analyst data |
| Yahoo Finance price history (via yfinance) | https://finance.yahoo.com/quote/AMD/history | SMA/RSI/ATR/performance calculations |
| Yahoo Finance news feed | https://finance.yahoo.com/quote/AMD/news | Catalyst headlines |
| Method template | AEHR.research.31.05.2026.pdf (user's Cowork method) | Report structure, scoring areas, checklist |

_Note: the template's native Finviz/Fintel/ADVFN pages block automated retrieval; equivalent metrics above are computed from Yahoo Finance data instead and labeled accordingly._