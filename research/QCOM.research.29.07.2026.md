# QCOM Research Report
**QUALCOMM Incorporated** — Technical + Fundamental Analysis, Data Snapshot, Institutional Investors
Signed file: `QCOM.research.29.07.2026`

| Field | Value |
|---|---|
| Current price | $155.68 (2026-07-29, ~15-min delayed) |
| Market cap | $164.09B |
| Sector / Industry | Technology / Semiconductors |
| Main theme | Semiconductors — see catalysts below |
| Current stance | Moderate technical momentum, mixed fundamentals, value: reasonable, risk: high. |

> Research journal, not financial advice. For education and decision journaling only — not a recommendation to buy, hold, or sell. Public Yahoo Finance data retrieved 2026-07-29.

## 1. Executive Summary
| Area | Score / Read | Reason |
|---|---|---|
| Technical momentum | Moderate | 1Y -1.30%; price -7.29% vs SMA200. |
| Fresh setup quality | Poor / broken | -37.75% from 52w high — base needs to rebuild. |
| Fundamental quality | Mixed | Profit margin 22.31%, revenue growth -3.50%. |
| Value attractiveness | Reasonable | Forward P/E 14.16, EV/Sales 3.98. |
| Risk level | High | Beta 1.64, ATR 6.3% of price, short float 3.84%. |

**Bottom line:** Moderate technical momentum, mixed fundamentals, value: reasonable, risk: high.

**Key questions before any trade (standard):**
- Is the stock forming a new tight base, or is it simply extended after news-driven gaps?
- Where is the actual invalidation level? If that level is too far away, the setup is not clean.
- Is the move supported by new institutional demand or just retail/short-covering momentum?
- Can earnings convert fast enough to justify the valuation?

## 2. Company Overview and Recent Catalysts
QUALCOMM Incorporated engages in the development and commercialization of foundational technologies for the wireless industry worldwide. It operates through three segments: Qualcomm CDMA Technologies (QCT); Qualcomm Technology Licensing (QTL); and Qualcomm Strategic Initiatives (QSI).

**Recent headlines (potential catalysts):**

| Headline | Source | Date |
|---|---|---|
| Earnings live: Qualcomm stock slides, Arm Holdings little changed after results exceed expectations | Yahoo Finance | 2026-07-27 |
| Qualcomm Q3 earnings top revenue expectations as smartphone market slows | Yahoo Finance | 2026-07-29 |
| Qualcomm and Arm Forecasts Fail to Impress Investors | Bloomberg | 2026-07-29 |
| Qualcomm’s Shares Fall After Earnings Miss | Barrons.com | 2026-07-29 |

## 3. Technical Analysis
| Technical item | Read |
|---|---|
| Price vs 52-week range | Close $155.68; 52w high $250.10 (-37.75%); 52w low $123.61 (+25.94%) |
| Trend | -7.29% vs SMA200, -22.28% vs SMA50, -11.62% vs SMA20 |
| Momentum | RSI(14) 32.3 (neutral) |
| Volatility | ATR(14) 9.76 (~6.3% of price); beta 1.64 |
| Setup perspective | -37.75% from 52w high — base needs to rebuild. |

**Performance snapshot:**

| Window | Return |
|---|---|
| Week | -11.4% |
| Month | -17.5% |
| Quarter | +4.2% |
| Half Y | +2.8% |
| 1Y | -1.3% |
| YTD | -9.1% |

## 4. Fundamental Analysis
| Factor | Observation | Implication / extra |
|---|---|---|
| Next earnings | 2026-07-29 | Next scheduled report (Yahoo estimate) |
| Revenue (ttm) | $44.49B | Revenue growth -3.50% y/y |
| Profitability | Gross 54.80%, operating 22.06%, net 22.31% | ROA 12.69%, ROE 36.08% |
| Balance sheet | Cash $9.80B, debt $15.27B | Current ratio 2.37, debt/equity 55.98 |
| Valuation | P/E 16.74, forward P/E 14.16, P/S 3.69, P/B 6.04 | EV/Sales 3.98, EV/EBITDA 13.63 |
| Growth expectations | Earnings growth 173.00%, EPS q/q 162.10% | Analyst mean target $220.57 (30 analysts) |

## 5. Institutional Investors and Ownership
| Metric | Value |
|---|---|
| Institutional ownership | 84.34% |
| Insider ownership | 0.12% |
| Short float | 3.84% |
| Short ratio (days to cover) | 1.8 |

**Top institutional holders:**

| Holder | Shares | % Out | Reported |
|---|---|---|---|
| Blackrock Inc. | 106,015,881 | 10.06% | 2026-03-31 |
| Vanguard Capital Management LLC | 69,341,312 | 6.58% | 2026-03-31 |
| State Street Corporation | 52,091,726 | 4.94% | 2026-03-31 |
| Invesco Ltd. | 35,245,336 | 3.34% | 2026-03-31 |
| Vanguard Portfolio Management LLC | 34,192,863 | 3.24% | 2026-03-31 |
| Charles Schwab Investment Management, Inc. | 30,864,702 | 2.93% | 2026-03-31 |
| Geode Capital Management, LLC | 29,734,061 | 2.82% | 2026-03-31 |
| Morgan Stanley | 19,991,313 | 1.90% | 2026-03-31 |
| Van Eck Associates Corporation | 14,022,899 | 1.33% | 2026-03-31 |
| UBS AM, a distinct business unit of UBS ASSET MANAGEMENT AMERICAS LLC | 12,113,406 | 1.15% | 2026-03-31 |

## 6. Risk Review and Setup Plan
- **Volatility risk:** Beta 1.64, ATR 6.3% of price, short float 3.84%. Size positions accordingly.
- **Trend risk:** far below the 52-week high; falling-knife entries without a base are low-quality setups.
- **Macro/sector risk:** semis are high-beta to AI capex sentiment, rates, and export-control headlines.

### Setup checklist for your journal
| Question | Pass condition |
|---|---|
| Is there a new base? | Tight sideways action near highs, declining volume, clear resistance. |
| Is the pivot obvious? | A clean level that multiple traders can see; not a random candle high. |
| Is risk defined? | Stop based on structure and less than the expected first upside target. |
| Is volume confirming? | Breakout volume expands; pullback volume dries up. |
| Is position size small enough? | A normal shakeout should not cause emotional decision-making. |

## 7. Full Data Snapshot
| Metric | Value | Metric | Value |
|---|---|---|---|
| Price | $155.68 | EV/Sales | 3.98 |
| Market cap | $164.09B | EV/EBITDA | 13.63 |
| Beta | 1.64 | Gross margin | 54.80% |
| RSI(14) | 32.3 | Operating margin | 22.06% |
| ATR(14) | 9.76 | Profit margin | 22.31% |
| SMA20 dist | -11.62% | ROA | 12.69% |
| SMA50 dist | -22.28% | ROE | 36.08% |
| SMA200 dist | -7.29% | Revenue (ttm) | $44.49B |
| 52W high | $250.10 | Revenue growth y/y | -3.50% |
| 52W low | $123.61 | Inst. ownership | 84.34% |
| P/E (ttm) | 16.74 | Insider ownership | 0.12% |
| Forward P/E | 14.16 | Short float | 3.84% |
| PEG (trailing) | 0.51 | Avg volume | 22,425,781 |
| P/S | 3.69 | Employees | 52,000 |
| P/B | 6.04 | Analyst rec (1=buy..5=sell) | 2.6 |

## 8. Analyst Actions
| Date | Action | Firm | Rating change |
|---|---|---|---|
| 2026-07-13 | main | TD Cowen | Buy → Buy |
| 2026-06-29 | main | Mizuho | Neutral → Neutral |
| 2026-06-25 | main | Barclays | Underweight → Underweight |
| 2026-06-25 | main | UBS | Neutral → Neutral |
| 2026-06-25 | main | Citigroup | Neutral → Neutral |
| 2026-06-25 | up | Morgan Stanley | Underweight → Equal-Weight |
| 2026-06-25 | main | Bernstein | Market Perform → Market Perform |
| 2026-06-25 | main | Wells Fargo | Equal-Weight → Equal-Weight |

## 9. Conclusion
QCOM: Moderate momentum / mixed fundamentals / reasonable value / high risk. Per the journal method: act only on a defined setup (new tight base, controlled pullback, or confirmed reclaim of a key level) with risk sized in advance — never chase an extended move.

## Sources
| Source | URL | Used for |
|---|---|---|
| Yahoo Finance quote/profile/statistics for QCOM | https://finance.yahoo.com/quote/QCOM | Snapshot metrics, ownership, analyst data |
| Yahoo Finance price history (via yfinance) | https://finance.yahoo.com/quote/QCOM/history | SMA/RSI/ATR/performance calculations |
| Yahoo Finance news feed | https://finance.yahoo.com/quote/QCOM/news | Catalyst headlines |
| Method template | AEHR.research.31.05.2026.pdf (user's Cowork method) | Report structure, scoring areas, checklist |

_Note: the template's native Finviz/Fintel/ADVFN pages block automated retrieval; equivalent metrics above are computed from Yahoo Finance data instead and labeled accordingly._