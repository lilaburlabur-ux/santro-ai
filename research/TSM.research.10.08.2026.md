# TSM Research Report
**Taiwan Semiconductor Manufacturing Company Limited** — Technical + Fundamental Analysis, Data Snapshot, Institutional Investors
Signed file: `TSM.research.10.08.2026`

| Field | Value |
|---|---|
| Current price | $418.47 (2026-08-10, ~15-min delayed) |
| Market cap | $2.17T |
| Sector / Industry | Technology / Semiconductors |
| Main theme | Semiconductors — see catalysts below |
| Current stance | High technical momentum, strong fundamentals, value: reasonable, risk: elevated. |

> Research journal, not financial advice. For education and decision journaling only — not a recommendation to buy, hold, or sell. Public Yahoo Finance data retrieved 2026-08-10.

## 1. Executive Summary
| Area | Score / Read | Reason |
|---|---|---|
| Technical momentum | High | 1Y +74.39%; price +16.37% vs SMA200. |
| Fresh setup quality | Moderate / wait | -12.38% from 52w high; no clean fresh pivot by default. |
| Fundamental quality | Strong | Profit margin 49.92%, revenue growth 36.00%. |
| Value attractiveness | Reasonable | Forward P/E 19.23, EV/Sales 3.36. |
| Risk level | Elevated | Beta 1.26, ATR 4.0% of price, short float 0.69%. |

**Bottom line:** High technical momentum, strong fundamentals, value: reasonable, risk: elevated.

**Key questions before any trade (standard):**
- Is the stock forming a new tight base, or is it simply extended after news-driven gaps?
- Where is the actual invalidation level? If that level is too far away, the setup is not clean.
- Is the move supported by new institutional demand or just retail/short-covering momentum?
- Can earnings convert fast enough to justify the valuation?

## 2. Company Overview and Recent Catalysts
Taiwan Semiconductor Manufacturing Company Limited, together with its subsidiaries, manufactures, packages, tests, and sells integrated circuits and other semiconductor devices in Taiwan, China, Europe, the Middle East, Africa, Japan, the United States, and internationally. It provides various wafer fabrication processes, such as processes to manufacture complementary metal- oxide-semiconductor (CMOS) logic, mixed-signal, radio frequency, embedded memory, bipolar CMOS mixed-signal, and others.

**Recent headlines (potential catalysts):**

| Headline | Source | Date |
|---|---|---|
| Chip equipment stocks rise after Taiwan Semiconductor revenue surges 45% | Yahoo Finance | 2026-08-10 |
| ASML Stock Rises 1.7% as AI Demand Signal Strengthens | GuruFocus.com | 2026-08-10 |
| Why Intel Stock Is Falling Today | GuruFocus.com | 2026-08-10 |
| Stock Market Today: Dow Dips, Nvidia Falls On This Report; Energy Play Clears Entry As Oil Pops (Live Coverage) | Investor's Business Daily | 2026-08-10 |

## 3. Technical Analysis
| Technical item | Read |
|---|---|
| Price vs 52-week range | Close $418.47; 52w high $477.57 (-12.38%); 52w low $224.84 (+86.12%) |
| Trend | +16.37% vs SMA200, -1.67% vs SMA50, +2.28% vs SMA20 |
| Momentum | RSI(14) 51.5 (neutral) |
| Volatility | ATR(14) 16.59 (~4.0% of price); beta 1.26 |
| Setup perspective | -12.38% from 52w high; no clean fresh pivot by default. |

**Performance snapshot:**

| Window | Return |
|---|---|
| Week | +3.0% |
| Month | -3.6% |
| Quarter | +1.9% |
| Half Y | +20.6% |
| 1Y | +74.4% |
| YTD | +31.6% |

## 4. Fundamental Analysis
| Factor | Observation | Implication / extra |
|---|---|---|
| Next earnings | 2026-10-15 | Next scheduled report (Yahoo estimate) |
| Revenue (ttm) | $4.44T | Revenue growth 36.00% y/y |
| Profitability | Gross 64.23%, operating 60.34%, net 49.92% | ROA 19.01%, ROE 39.97% |
| Balance sheet | Cash $3.52T, debt $982.45B | Current ratio 2.46, debt/equity 15.17 |
| Valuation | P/E 36.77, forward P/E 19.23, P/S 0.49, P/B 87.27 | EV/Sales 3.36, EV/EBITDA 4.70 |
| Growth expectations | Earnings growth 77.40%, EPS q/q 77.40% | Analyst mean target $540.20 (18 analysts) |

## 5. Institutional Investors and Ownership
| Metric | Value |
|---|---|
| Institutional ownership | 15.50% |
| Insider ownership | 0.04% |
| Short float | 0.69% |
| Short ratio (days to cover) | 2.3 |

**Top institutional holders:**

| Holder | Shares | % Out | Reported |
|---|---|---|---|
| FMR, LLC | 60,393,190 | 1.16% | 2026-03-31 |
| Capital World Investors | 32,886,937 | 0.63% | 2026-03-31 |
| Sanders Capital, LLC | 28,060,092 | 0.54% | 2026-03-31 |
| JPMORGAN CHASE & CO | 23,984,642 | 0.46% | 2026-03-31 |
| Blackrock Inc. | 20,112,852 | 0.39% | 2026-06-30 |
| Van Eck Associates Corporation | 19,262,004 | 0.37% | 2026-03-31 |
| Fisher Asset Management, LLC | 18,866,198 | 0.36% | 2026-06-30 |
| Morgan Stanley | 14,767,680 | 0.28% | 2026-03-31 |
| Bank of America Corporation | 14,107,771 | 0.27% | 2026-03-31 |
| Capital International Investors | 11,306,653 | 0.22% | 2026-03-31 |

## 6. Risk Review and Setup Plan
- **Volatility risk:** Beta 1.26, ATR 4.0% of price, short float 0.69%. Size positions accordingly.
- **Macro/sector risk:** semis are high-beta to AI capex sentiment, rates, and export-control headlines.

### Setup checklist for your journal
| Question | Pass condition |
|---|---|
| Is there a new base? | Tight sideways action near highs, declining volume, clear resistance. |
| Is the pivot obvious? | A clean level that multiple traders can see; not a random candle high. |
| Is risk defined? | Stop based on structure and less than the expected first upside target. |
| Is volume confirming? | Breakout volume expands; pullback volume dries up. |
| Is position size small enough? | A normal shakeout should not cause emotional decision-making. |

## 7. Full Data Snapshot
| Metric | Value | Metric | Value |
|---|---|---|---|
| Price | $418.47 | EV/Sales | 3.36 |
| Market cap | $2.17T | EV/EBITDA | 4.70 |
| Beta | 1.26 | Gross margin | 64.23% |
| RSI(14) | 51.5 | Operating margin | 60.34% |
| ATR(14) | 16.59 | Profit margin | 49.92% |
| SMA20 dist | +2.28% | ROA | 19.01% |
| SMA50 dist | -1.67% | ROE | 39.97% |
| SMA200 dist | +16.37% | Revenue (ttm) | $4.44T |
| 52W high | $477.57 | Revenue growth y/y | 36.00% |
| 52W low | $224.84 | Inst. ownership | 15.50% |
| P/E (ttm) | 36.77 | Insider ownership | 0.04% |
| Forward P/E | 19.23 | Short float | 0.69% |
| PEG (trailing) | 1.01 | Avg volume | 14,129,935 |
| P/S | 0.49 | Employees | 76,907 |
| P/B | 87.27 | Analyst rec (1=buy..5=sell) | 1.4 |

## 8. Analyst Actions
| Date | Action | Firm | Rating change |
|---|---|---|---|
| 2026-07-27 | main | Needham | Buy → Buy |
| 2026-07-17 | main | DA Davidson | Buy → Buy |
| 2026-07-17 | main | TD Cowen | Hold → Hold |
| 2026-07-17 | main | Barclays | Overweight → Overweight |
| 2026-07-16 | main | Susquehanna | Positive → Positive |
| 2026-06-24 | main | B of A Securities | Buy → Buy |
| 2026-06-22 | main | Susquehanna | Positive → Positive |
| 2026-04-22 | main | Barclays | Overweight → Overweight |

## 9. Conclusion
TSM: High momentum / strong fundamentals / reasonable value / elevated risk. Per the journal method: act only on a defined setup (new tight base, controlled pullback, or confirmed reclaim of a key level) with risk sized in advance — never chase an extended move.

## Sources
| Source | URL | Used for |
|---|---|---|
| Yahoo Finance quote/profile/statistics for TSM | https://finance.yahoo.com/quote/TSM | Snapshot metrics, ownership, analyst data |
| Yahoo Finance price history (via yfinance) | https://finance.yahoo.com/quote/TSM/history | SMA/RSI/ATR/performance calculations |
| Yahoo Finance news feed | https://finance.yahoo.com/quote/TSM/news | Catalyst headlines |
| Method template | AEHR.research.31.05.2026.pdf (user's Cowork method) | Report structure, scoring areas, checklist |

_Note: the template's native Finviz/Fintel/ADVFN pages block automated retrieval; equivalent metrics above are computed from Yahoo Finance data instead and labeled accordingly._