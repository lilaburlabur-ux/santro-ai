...
(function (root, factory) {
  ...
  function parse(text){
    ...
    (text||"").split(/[\n,;]+/).forEach(chunk=>{
      ...
      const m=s.match(/^([A-Za-z][A-Za-z0-9.\-]{0,9})\s*[:\s]?\s*([\d.]+)?\s*%?$/);
  ...
  return {parse, classify, run, compressForModel, SCENARIOS, EXPOSURE_MAP, DISCLAIMER};
});
